...
def images():
    ...
    Alien.images = load_images('vacheline3.gif')
    ...
    Background.image = pygame.transform.scale(load_image('desert.png'), (ECRAN.width, ECRAN.height))
...
def main():
    ...
    while player1.alive() or player2.alive():
        ...
        if (player2.alive()):
            for alien in pygame.sprite.spritecollide(player2, aliens, 1):
                Explosion(alien)
                Explosion(player2)
                score.point()
                player2.kill()

        for alien in pygame.sprite.groupcollide(aliens, shots1, 0, 1).keys():
                alien.feu()
                if (not alien.alive()):
                    Explosion(alien)
                    score.point()

        for alien in pygame.sprite.groupcollide(aliens, shots2, 0, 1).keys():
            alien.feu()
            if (not alien.alive()):
                Explosion(alien)
                score.point()
